...
const PROXIES = [
  ...
  { build: (u) => `https://cors.eu.org/${u}`, parse: (t) => JSON.parse(t) },
  ...
  { build: (u) => `https://everyorigin.jwvbremen.nl/api/get?url=${encodeURIComponent(u)}`, parse: (t) => JSON.parse(JSON.parse(t).contents) },
...
async function tryFetch(proxy, url, timeoutMs = 6500) {
...
async function fetchJSON(path, onProgress) {
  ...
  if (workingProxyIndex !== null) {
    try {
      const data = await tryFetch(PROXIES[workingProxyIndex], url);
      cacheSet(path, data);
      return data;
    } catch (e) {
  ...
  // Every relay failed — fall back to the last good copy we cached
  // on a previous successful visit, so the page still shows real
  // data instead of a dead end.
...
async function boot(teamId, attempt = 0) {
  ...
  try {
    ...
    el.innerHTML =
      `<span>Couldn't reach the FPL API through any of the ${PROXIES.length} available routes right now. This can happen if a browser extension (ad-blocker/privacy blocker) is blocking proxy domains, or if the mobile network is restrictive — try switching between Wi-Fi and mobile data, or disabling content blockers for this site, then retry. Technical detail: ${esc(e.message)}</span>` +
